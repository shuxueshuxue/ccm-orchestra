...
- `tmux` layer: the real Claude agent session. This is the default path.
...
Also do not confuse the wakeup model:

- `ccm read` is poll-based waiting on Claude transcript output
- `ccm relay` is push-based messaging into another visible tab

If you need another agent tab to wake up later, `relay` is the right primitive. `read` will never do that for you.
...
Visible Codex tabs now get one extra Enter retry on relay delivery. Treat that as probability reduction for rare submit misses, not a mathematical guarantee.
...
If an agent suddenly starts producing repeated `502` errors after a Claude or `ccm` upgrade, check whether the agent was started earlier with a stale Claude binary inside tmux.

The failure mode we hit in practice:

- current shell: `~/.cac/bin/claude` on a newer version
- existing tmux agent pane: old `/opt/homebrew/bin/claude`

That agent must be restarted. `which claude` in the current shell does not prove the running agent is current.
...
Only open the agent into a visible `kitty` tab if transcript output is not enough:
...
```bash
ccm send frontend-agent "You are helping on the current branch in $PWD. Review only the frontend/UI surfaces affected by this branch. Identify the current UX and visual weaknesses, then propose 2-3 better implementations. Stay minimal and branch-scoped."
...
```bash
ccm send frontend-agent "Pick the best proposal and turn it into a concrete implementation plan with file-level suggestions."
ccm read frontend-agent --wait-seconds 30
...
```bash
ccm send frontend-agent "I have updated the branch. Critique the result strictly. What still feels weak, confusing, or visually flat? Keep the feedback branch-scoped."
ccm read frontend-agent --wait-seconds 30
```
