...
import codex_heartbeat


class HeartbeatPathTests(unittest.TestCase):
    def test_state_paths_are_scoped_by_tab_title_slug(self):
        with tempfile.TemporaryDirectory() as tmp, mock.patch.object(codex_heartbeat, "STATE_DIR", Path(tmp)):
            pid_path = codex_heartbeat.heartbeat_pid_path("Feature Main")
            log_path = codex_heartbeat.heartbeat_log_path("Feature Main")
...
class HeartbeatDeliveryTests(unittest.TestCase):
    @mock.patch("codex_heartbeat.subprocess.run", autospec=True)
    @mock.patch("codex_heartbeat.resolve_tab_window_id", autospec=True, return_value=777)
    def test_send_heartbeat_targets_custom_tab_title(self, resolve_tab_window_id, run):
        window_id = codex_heartbeat.send_heartbeat("unix:/tmp/mykitty", "hello", "Feature Main")
    ...
    @mock.patch("codex_heartbeat.send_heartbeat", autospec=True, return_value=888)
    def test_test_once_reports_sent_window(self, send_heartbeat):
        with mock.patch("sys.stdout.write") as write:
            exit_code = codex_heartbeat.test_once("unix:/tmp/mykitty", "ping", "Feature Main")
...
class HeartbeatLifecycleTests(unittest.TestCase):
    @mock.patch("codex_heartbeat.subprocess.Popen", autospec=True)
    @mock.patch("codex_heartbeat.resolve_tab_window_id", autospec=True, return_value=777)
    def test_start_background_passes_tab_title_and_uses_scoped_log(self, resolve_tab_window_id, popen):
        ...
        with tempfile.TemporaryDirectory() as tmp, \
             mock.patch.object(codex_heartbeat, "STATE_DIR", Path(tmp)), \
             mock.patch("codex_heartbeat.time.sleep"), \
             mock.patch("codex_heartbeat.pid_is_alive", side_effect=[False, True]):
            exit_code = codex_heartbeat.start_background("unix:/tmp/mykitty", 30, "hello", "Feature Main")
    ...
    def test_status_background_uses_scoped_paths(self):
        with tempfile.TemporaryDirectory() as tmp, mock.patch.object(codex_heartbeat, "STATE_DIR", Path(tmp)):
            ...
            with mock.patch("codex_heartbeat.pid_is_alive", return_value=True), \
                 mock.patch("sys.stdout.write") as write:
                exit_code = codex_heartbeat.status_background(tab_title="Feature Main")
