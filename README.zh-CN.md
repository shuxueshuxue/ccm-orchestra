...
`ccm-orchestra` 是一个控制层，用来在 `tmux` 里运行持续的交互式 Claude Code 智能体，并通过 `kitty` 协调不同 code agent 之间的可见协作。两者构成一个配对操作模型：`tmux` 让智能体持续存活、可复用，`kitty` 让协作过程可见并支持 relay。

核心 loop 很简单：在 detached tmux pane 里启动一个智能体，给它发 prompt，再从 transcript 里读新输出。Session 按工作目录隔离，所以不同仓库里可以复用同一个 agent 名。`tmux` 层和 `kitty` 层合在一起，才让 Claude Code、Codex 以及其他 code agent 能并行协作、互相交接、并在需要时保持可观察。

## 两层结构

这个系统本质上分成两层：

- `tmux` 层：真正的会话层。负责把交互式 Claude Code 智能体持续跑着，按 worktree 隔离，并让 Codex 反复复用同一个智能体。
- `kitty` 层：可见协作层。负责把部分会话拉到前台可见，列出可见 peer，并在 tab 之间做带回执约定的通信。

如果只记一件事，就记这个：

- `tmux` 和 `kitty` 是两个并列能力，可以单独用，也可以联动用
- 默认智能体 loop 在 `tmux` 层：`start -> send -> read`
...
- 同一个交互式智能体可以同时被两边接触：人可以 attach/观察，程序化工具也仍然可以 send、read、doctor、restart、supervise
...
只使用全局 `ccm`。如果 Claude 或 `ccm` 升级后 agent 开始异常：
...
- 每个可见的 Codex tab 都应该在 tmux 里维护一个长期存活、值得信任的专属 Claude agent，反复复用。不要每做完一件小事就 kill 掉 agent。持久 session 本身就是核心价值。
- agent 名称按职责取，并尽量具体，比如 `frontend-agent`、`docs-editor`。不要在当前 namespace 里重复使用一个含糊又容易撞名的名字。
...
`kitty` 是可见协作层。它不是 agent 真正运行的那一层，但当人类、Codex、Claude 需要互相看见并实时交换消息时，它就是系统里的第一等能力。
...
可见 tab 不需要 registry，headless agent 也不需要假 alias。直接写目标：
...
如果是 headless 的 Claude/tmux agent，就直接写 tmux session，不要为了 WeChat 临时再开一个可见 kitty tab：
